...
from ..database import SessionLocal, get_db, pool_status
...
def _close_answer_if_still_open_sync(event_id: UUID, question_id: UUID, deadline: datetime) -> None:
    worker_started_at = datetime.now(timezone.utc)
    auto_close_lag_ms = (worker_started_at - deadline).total_seconds() * 1000
    db = SessionLocal()
    try:
        event = db.get(Event, event_id)
        if event is None:
        ...
        if event.phase == QuizPhase.ANSWER_OPEN and event.current_question_id == question_id:
            ...
            db.commit()
            db.refresh(event)
            commit_completed_at = time.perf_counter()
            logger.info(
                "PERF auto_close event_id=%s question_id=%s deadline=%s worker_started_at=%s "
                "auto_close_lag_ms=%.3f",
                event_id,
                question_id,
                deadline.isoformat(),
            ...
            _broadcast_current_state(db, event, commit_completed_at)
    finally:
        db.close()
...
async def _auto_close_after_deadline(event_id: UUID, question_id: UUID, deadline: datetime) -> None:
    ...
    await asyncio.to_thread(_close_answer_if_still_open_sync, event_id, question_id, deadline)
...
def _commit_answer_start(db: Session, event: Event, question: Question) -> None:
    ...
    _broadcast_current_state(db, event, time.perf_counter())
